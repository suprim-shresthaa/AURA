...
const ManageReservations = () => {
  ...
  const getItemName = (reservation) => {
    if (reservation.bookingType === "vehicle" && reservation.vehicle) {
      ...
      reservation.bookingType === "sparePart" &&
      reservation.sparePart
    ) {
  ...
  const getItemLink = (reservation) => {
    if (reservation.bookingType === "vehicle") {
      return `/vehicles/${reservation.vehicle?.id}`;
    } else if (reservation.bookingType === "sparePart") {
      return `/spare-parts/${reservation.sparePart?.id}`;
    }
    return "#";
  };
  ...
  const getItemIcon = (reservation) => {
    if (reservation.bookingType === "vehicle") {
      ...
      return (
        <img
          src={reservation.sparePart?.images[0]}
          alt={reservation.sparePart?.name}
          className="w-10 h-10 object-cover rounded-lg"
        />
      );
    }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 p-4">
        ...
        <p className="text-sm text-gray-600">
          Manage all vehicle and spare part bookings
        </p>
      </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              {reservation.insuranceSelected && (
                                <p className="text-xs text-gray-500">
                                  Insurance: Rs.{" "}
                                  {reservation.insuranceAmount || 500}
                                </p>
                              )}
                            </div>
                          </div>
