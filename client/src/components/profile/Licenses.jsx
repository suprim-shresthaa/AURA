...
const ALLOWED_LICENSE_IMAGE_TYPES = ["image/jpeg", "image/png"];
...
const Licenses = () => {
  ...
  const handleImageChange = (e) => {
    ...
    if (!ALLOWED_LICENSE_IMAGE_TYPES.includes(file.type)) {
  ...
  const handleUpload = async (e) => {
    ...
    if (selectedVehicleTypes.length === 0 || !licenseImage) {
      toast.error(
        "Please select at least one vehicle type and upload a license image.",
      );
      return;
    }
    ...
    try {
      ...
      if (response.data?.success) {
        const message =
          ...
          "License uploaded successfully! It will be reviewed by an admin.";
  ...
  const availableVehicleTypes = vehicleTypes.filter(
    (type) =>
      !licenses.some(
        (license) =>
          license.vehicleType === type && license.status !== "rejected",
      ),
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage your driving licenses for different vehicle types
          </p>
        </div>
        {availableVehicleTypes.length > 0 && !showUploadForm && (
          <button
            onClick={() => setShowUploadForm(true)}
            className="bg-primary text-white py-2 px-4 rounded-md flex items-center hover:bg-primary-700 cursor-pointer"
          >
            ...
            <form onSubmit={handleUpload} className="space-y-4">
                ...
                <div className="space-y-2 border border-gray-300 rounded-lg p-3 bg-gray-50">
                  {availableVehicleTypes.length === 0 ? (
                    <p className="text-sm text-gray-500">
                      All vehicle types already have licenses.
                    </p>
                  ) : (
                    availableVehicleTypes.map((type, index) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          required={
                            index === 0 && selectedVehicleTypes.length === 0
                          }
              ...
              <div className="flex gap-3 justify-end pt-4 border-t">
                ...
                <button
                  type="submit"
                  disabled={uploading}
                  className="px-4 py-2 bg-primary text-white cursor-pointer rounded-md hover:bg-primary/90 flex items-center"
                >
                  {uploading ? (
                    <>
          ...
          <div className="pt-6">
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">
                Upload your driving license. You can use the same license image
                for multiple vehicle types.
              </p>
              {availableVehicleTypes.length > 0 && (
                <button
                  onClick={() => setShowUploadForm(true)}
                  className="bg-primary text-white px-4 py-2 rounded-md hover:bg-primary/90 flex items-center justify-center mx-auto"
                >
                  <Upload className="mr-2 h-4 w-4" />
        ...
        <div className="grid gap-4">
          {groupedLicenses.map((group) => {
            ...
            const primaryLicense = group.licenses.reduce((prev, curr) => {
              return getStatusPriority(curr.status) >
                getStatusPriority(prev.status)
                ? curr
                : prev;
            });

            const statusInfo =
              statusConfig[primaryLicense.status] || statusConfig.pending;
            const StatusIcon = statusInfo.icon;

            // Check if all licenses are rejected
            const allRejected = group.licenses.every(
              (l) => l.status === "rejected",
            );
            const rejectedLicenses = group.licenses.filter(
              (l) => l.status === "rejected",
            );
            const rejectionReasons = [
              ...new Set(
                rejectedLicenses.map((l) => l.rejectionReason).filter(Boolean),
              ),
            ];
            ...
            return (
              <div
                key={group.key}
                className="overflow-hidden bg-white shadow sm:rounded-lg p-6"
              >
                <div>
                    ...
                    <div className="space-y-4">
                      ...
                      {group.licenses.some((l) => l.approvedAt) && (
                        <div className="text-sm text-gray-600">
                          ...
                          <p>
                            {new Date(
                              group.licenses.find((l) => l.approvedAt)
                                ?.approvedAt,
                            ).toLocaleString()}
                          </p>
                        </div>
                        ...
                        <div className="text-sm text-gray-600">
                          ...
                          <div className="space-y-1">
                            {group.licenses.map((license) => {
                              const licenseStatusInfo =
                                statusConfig[license.status] ||
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <span
                                    className={`px-2 py-1 rounded text-xs flex items-center gap-1 ${licenseStatusInfo.className}`}
                                  >
