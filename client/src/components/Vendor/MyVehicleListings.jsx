...
export default function MyVehicleListings() {
  ...
  const fetchVehicles = async (id) => {
    try {
      ...
      const res = await fetch(
        `http://localhost:5001/api/vehicles/vendor-vehicles?vendorId=${id}`,
      );
  ...
  const confirmDelete = async () => {
    if (!vehicleToDelete || !vendorId) return;
    setDeleting(true);
    try {
      const params = new URLSearchParams({ vendorId: String(vendorId) });
      const res = await fetch(
        `http://localhost:5001/api/vehicles/${vehicleToDelete}?${params.toString()}`,
        {
          method: "DELETE",
  ...
  const handleToggleAvailability = async (vehicleId, currentAvailability) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5001/api/vehicles/${vehicleId}/availability`,
        {
          method: "PUT",
          headers: {
  ...
  return (
    <div className="flex min-h-screen  bg-gray-50">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`px-4 py-2.5 rounded-lg font-medium transition-colors ${filterStatus === "all" ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}
                >
                  All
                </button>
                <button
                  onClick={() => setFilterStatus("available")}
                  className={`px-4 py-2.5 rounded-lg font-medium transition-colors ${filterStatus === "available" ? "bg-green-600 text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}
                >
                  Available
                </button>
                <button
                  onClick={() => setFilterStatus("rented")}
                  className={`px-4 py-2.5 rounded-lg font-medium transition-colors ${filterStatus === "rented" ? "bg-orange-600 text-white" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}
                >
                  Rented
                </button>
              </div>
                    ...
                    <div className="col-span-2">
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <span
                            className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${vehicle.isAvailable ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                          >
      ...
      <ConfirmationModal
        ...
        title="Delete Vehicle"
        message="This action cannot be undone. The vehicle will be removed from your listings. If there is an active or upcoming rental, deletion is blocked until that booking ends or is cancelled."
        confirmText="Delete"
        cancelText="Cancel"
        type="danger"
        loading={deleting}
