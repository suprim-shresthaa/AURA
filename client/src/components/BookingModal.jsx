...
export default function BookingModal({ isOpen, onClose, vehicle, sparePart }) {
  ...
  const fetchBookedRanges = async () => {
    ...
    try {
      ...
      const params = isSparePart
        ? { sparePartId: sparePart._id, startDate: today, endDate }
        : { vehicleId: vehicle._id, startDate: today, endDate };
    ...
    try {
      ...
      if (response.data?.success) {
        const approvedLicense = (response.data.data || []).find(
          (license) =>
            license.vehicleType === vehicle.category &&
  ...
  const isDateInBookedRanges = (dateString) => {
    if (!dateString || !bookedRanges.length) return false;
  ...
  const isRangeConflicting = (startDate, endDate) => {
    if (!startDate || !endDate || !bookedRanges.length) return false;
  ...
  const handleDateRangeChange = (_, dateStrings) => {
    ...
    if (startDate && endDate) {
      if (isRangeConflicting(startDate, endDate)) {
        ...
        setError(
          "Selected dates overlap with booked dates. Please choose another range.",
        );
        return;
      }
  ...
  const handleNext = async () => {
    ...
    if (isRangeConflicting(formData.startDate, formData.endDate)) {
      setError(
        "Selected dates overlap with booked dates. Please choose another range.",
      );
      return;
            ...
            {[1, 2].map((s) => (
              <div key={s} className="flex items-center flex-1">
                <div className="flex items-center">
                  ...
                  <span
                    className={`ml-2 text-sm font-medium ${step >= s ? "text-blue-600" : "text-gray-600"}`}
                  >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-yellow-700 mb-3">
                        You need an approved {vehicle?.category} license to book
                        this vehicle.
                      </p>
                      <Button
                        onClick={() => {
            ...
            <div className="space-y-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">
                      {isSparePart ? "Spare Part" : "Vehicle"}:
                    </span>
                  ...
                  <label className="inline-flex items-start gap-3">
                    ...
                    <div>
                      <div className="font-medium">
                        Add Insurance (Accidental coverage)
                      </div>
                      <div className="text-sm text-gray-600">
                        Rs. 500 — covers accidental damage during rental period.
                      </div>
                    </div>
                  </label>
                </div>
                ...
                <div className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                  <label className="inline-flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-600">
                        Rs. 300 per day — professional driver for your rental
                        period.
                      </div>
                    </div>
                  </label>
              ...
              <p className="text-sm text-red-600">
                Note: once vehicle and spare parts book cannot be cancel.
              </p>
                ...
                {insuranceSelected && (
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e) =>
                        setAgreedToInsuranceTerms(e.target.checked)
                      }
                      className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <label
                      htmlFor="agreeInsuranceTerms"
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex gap-3 justify-end">
          ...
          {step < 2 ? (
            ...
            <Button
              ...
              disabled={
                loading ||
                (insuranceSelected && !agreedToInsuranceTerms) ||
